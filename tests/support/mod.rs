...
//! `tokio::net` (the house pattern for integration tests, ported from
//! unidpp-registry's `support` — itself ported from unidpp-resolver's
//! `httpc`; no reqwest/hyper-client/TLS dependency). `http://` only;
//! integration servers bind loopback.
...
pub async fn request(
    ...
    let fut = async {
        let (host, port, path_and_query) = split_url(url)?;
...
fn split_url(url: &str) -> std::io::Result<(String, u16, String)> {
    ...
    let (scheme, rest) = url
        .split_once("://")
        .ok_or_else(|| invalid(&format!("URL must be absolute: `{url}`")))?;
    ...
    let (host, port) = match authority.rsplit_once(':') {
        Some((h, p)) => (
            h.to_string(),
            p.parse::<u16>().map_err(|_| invalid("bad port"))?,
        ),
